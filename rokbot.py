"""
ROK Unified Tool - Modern GUI Edition
...
class ModernROKTool:
    def __init__(self):
        ...
        self.root.resizable(True, True)  # Allow resize for better performance
        ...
        self.gui_update_interval = 0.5  # Update GUI every 0.5s instead of every action
        ...
        # Timing parameters - slower, more human-like
        self.click_randomization = 12
        self.micro_correction_chance = 25
        self.overshoot_chance = 15
        self.action_delay_min = 2500  # Increased from 1400
        self.action_delay_max = 4500  # Increased from 3200
        
        # Mouse movement delays (slower)
        self.mouse_move_delay_min = 25  # Increased from 15
        self.mouse_move_delay_max = 65  # Increased from 45
        self.bezier_points = 12  # More points = smoother, slower movement
        self.curve_intensity = 25  # Increased curve for more human-like path
        
        # Click delays (pause before/after click)
        self.pre_click_delay_min = 150  # Wait before clicking
        self.pre_click_delay_max = 350
        self.post_click_delay_min = 200  # Wait after clicking
    ...
    def setup_profile_coordinates(self):
        ...
        # Set active (initialize with Nhan as default)
    ...
    def add_gui_log(self, msg):
        """Add message to GUI log with throttling"""
        try:
    ...
    def create_autofarm_tab(self):
        """Create AutoFarm tab with scrollable content"""
    ...
    def create_resources_section(self, parent):
        """Create resource selection - compact"""
        frame = ctk.CTkFrame(parent)
    ...
    def create_features_section(self, parent):
        """Create features toggles - compact"""
        frame = ctk.CTkFrame(parent)
        frame.pack(fill="x", padx=5, pady=5)
    ...
    def create_webhook_section(self, parent):
        """Create webhook section - compact"""
        frame = ctk.CTkFrame(parent)
        frame.pack(fill="x", padx=5, pady=5)
    ...
    def create_activity_log(self, parent):
        """Create activity log - optimized for performance"""
        frame = ctk.CTkFrame(parent)
        ...
        # Reduce height for better performance
        self.log_textbox = ctk.CTkTextbox(
            frame,
            wrap="word",
            font=ctk.CTkFont(size=10, family="Consolas"),
            height=300,
            activate_scrollbars=True  # Only show scrollbars when needed
        )
        self.log_textbox.pack(fill="both", expand=True, padx=8, pady=(0, 8))
    
    def create_status_section(self, parent):
        """Create status indicators - compact"""
        frame = ctk.CTkFrame(parent)
    ...
    def is_gathering(self, x, y):
        ...
        try:
            ...
            check_patterns = [
                ...
                [(-4, -4), (4, -4), (-4, 4), (4, 4)],  # Diagonal
                [(-8, 0), (8, 0), (0, -8), (0, 8)],  # Outer ring
                [(-12, 0), (12, 0), (0, -12), (0, 12)]  # Extended
            ]
            ...
            # If 20% of checks match, troops are gathering
            if total_checks > 0 and (match_count / total_checks) >= 0.20:
                return True
            
            # Additional check with higher tolerance
    ...
    def check_help_button(self):
        ...
        # Check if help_area is initialized
    ...
    def check_reconnect(self):
        ...
        # Check if reconnect_area is initialized
    ...
    def advanced_mouse_move(self, target_x, target_y):
        """Advanced mouse movement with bezier curves - slower, more human"""
        if not AUTOGUI_AVAILABLE:
            return
        ...
        try:
            ...
            # Move along bezier curve with slower speed
            ...
            if should_overshoot:
                ...
                for i in range(1, 4):
                    ...
                    time.sleep(0.03)  # Slower correction
            
            # Micro correction
            if random.randint(1, 100) <= self.micro_correction_chance:
                time.sleep(random.uniform(0.05, 0.12))  # Longer pause
    ...
    def send_march(self):
        """Send a march with full slot checking - slower, more human timing"""
        if not AUTOGUI_AVAILABLE:
            return False
        ...
        try:
            ...
            # Update slot check time
            self.slot_last_check[available_slot] = time.time() * 1000
            
            resource = self.get_random_resource()
            self.log(f"March {available_slot + 1} - {resource} (Fatigue: {round(self.fatigue_level)}%)")
            ...
            time.sleep(random.uniform(1.2, 2.0))  # Wait for UI
            ...
            if self.is_gathering(self.march_slots[available_slot][0], self.march_slots[available_slot][1]):
                ...
                self.log(f"✅ March {available_slot + 1} SUCCESS - {resource}")
                
                if self.webhook_enabled and self.webhook_on_success:
                    self.send_webhook("success", f"March {available_slot + 1} SUCCESS - {resource}", 0x27AE60)
    ...
    def autofarm_loop(self):
        """Main AutoFarm loop with full detection"""
        while self.running:
            if not self.toggle:
                time.sleep(0.1)
                continue
    ...
    def update_stats(self):
        ...
        # Throttle GUI updates to reduce lag
...
def main():
    """Main entry point with performance tips"""
    print("=" * 60)
    print("ROK Unified Tool - Professional Edition")
    print("=" * 60)
